...
def test_embedding_non_streaming():
    ...
    print(f"Deployment: {embedding_deployment_name}")

    input_texts = [
        "This is some text to generate embeddings for", # dummy first entry to enable header diffs
        "This is some text to generate embeddings for",
        (
            "This is some text to generate embeddings for and is"
    ...
    # Use this loop for repeated requests to check limiting
    # for i in range(request_count):
    #     print(f"Making request {i}...")
    #     result = call_embedding_api_non_streaming(input_texts[0])
    #     results.append(result)
    #     if sleep_time > 0:
    #         time.sleep(sleep_time)

    # Use this loop to test different input texts (e.g. examine the rate-limit header deltas)
    for i, input_text in enumerate(input_texts):
        print(f"Making request {i}...")
        result = call_embedding_api_non_streaming(input_text)
        results.append(result)
        if sleep_time > 0:
            time.sleep(sleep_time)
